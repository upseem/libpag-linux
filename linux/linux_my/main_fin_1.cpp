...
int main(int argc, char* argv[]) {
  ...
  for (const auto& replacement : config["replacements"]) {
    ...
    if (type == "text") {
      ...
      //如果替换的是一张图片那么直接替换
      int totalFrames = replacement["totalFrames"].asInt();
      if (totalFrames == 1) {
  ...
  while (currentFrame <= totalFrames) {
    ...
    // 对每一段视频进行处理
    for (int i = 0; i < config["currentFrames"].size(); ++i) {
      Json::Value currentVideoFrames = config["currentFrames"][i];
      int imageIndex = currentVideoFrames[2].asInt();
      ...
      if (currentFrame >= currentVideoFrames[0].asInt() &&
        ...
        if (ifile) {
          ReplaceOneImage(pagFile.get(), imageIndex, imagePath);
        }
      }
    }
