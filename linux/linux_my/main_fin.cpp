...
int main(int argc, char *argv[])
{
  ...
  for (const auto &replacement : config["replacements"])
  {
    ...
    int currentVideo = 0; // 当前需要处理的图层index
    Json::Value currentVideoFrames =
        config["currentFrames"][currentVideo]; // 当前视频的帧范围 [1,131,0]

    int imageIndex = currentVideoFrames[2].asInt(); // 当前需要被替换的图像的索引 0

    // 当当前帧的序号小于或等于总帧数时，继续循环。
    while (currentFrame <= totalFrames)
    {

      // 如果超出当前视频的帧范围，更新视频索引并获取下一个视频的帧范围
      if (currentFrame > currentVideoFrames[1].asInt())
      {
        currentVideo++;
        currentVideoFrames = config["currentFrames"][currentVideo];
        imageIndex = currentVideoFrames[2].asInt(); // 更新需要被替换的图像的索引
      }
      std::cout << "imageIndex " << imageIndex << std::endl;

      // 如果在当前视频的帧范围内，进行图片替换
      if (currentFrame >= currentVideoFrames[0].asInt() &&
          currentFrame <= currentVideoFrames[1].asInt())
      {
